...
8. 原始音视频只作为临时处理文件，默认在任务完成或失败后删除；用户可关闭删除以便调试。
...
14. 默认 ASR 使用本地 faster-whisper；ASR 通过统一端口接入，可通过配置切换到腾讯云或其他供应商。
15. 默认媒体临时存储为可配置本地目录；选择腾讯云 ASR 时必须使用腾讯云 COS 中转，COS 逻辑封装在腾讯适配器内部。
...
- 任务状态、失败原因、重试机制和基础日志。
- 个人用户白名单和敏感配置管理。
- Docker Compose 应用部署、用户自建 PostgreSQL 连接和健康检查。
...
6. 原始音视频仅作为临时文件，按配置在任务结束后删除（默认删除）；完整转录文本必须保留。
...
- 优先匿名解析官方分享短链和公开详情页；需要登录态或触发验证时明确失败，不伪造完整正文。
- 图文笔记按页面顺序下载全部图片，经本地 Base64 完成 OCR 与视觉理解，全部成功后才标记完整。
- 视频选择最高分辨率媒体流并保留平台返回的备用 CDN，进入统一视频转录流程。
- 访问令牌和短期媒体 URL 不进入规范 URL、持久化结果或日志。
...
- 支持标准视频链接、`b23.tv` 分享短链和明确的分P参数。
- 提取 BV、CID、标题、UP 主、简介、发布时间、时长和匿名可访问媒体元数据。
- DASH 音视频分离时，转录链路直接下载最佳可用音频轨；需要完整视频文件时必须下载音视频双轨并正确封装。
- 主音频 CDN 下载失败时按平台返回的备用地址回退；不得使用 HEAD 结果替代真实流式 GET 可用性判断。
- 不持久化具有时效性的媒体直链；会员、地区限制、已删除或访问校验必须以可见失败结束。
...
- 选择腾讯云 ASR 时，适配器把规范音频分段上传到指定 COS Bucket 的知归专用前缀，通过短期预签名 HTTPS URL 提交识别；不得把 Bucket 公开读权限作为接入前提。
- 转录文本保留时间戳的能力作为 P1，MVP 可只保存纯文本。
- 支持中文及中英文混合内容。
- 转录结果需记录使用的模型、语言、时长和是否降级。
- 原始音视频不得作为归档资产长期保存；本地临时文件和腾讯模式下的 COS 临时对象按统一清理配置处理，默认在任务作用域结束时删除。
...
| 字段名 | 建议类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| 标题 | 单行文本 | 是 | AI 根据正文生成的简短标题，目标 8 至 20 个中文字符，最多 30 个字符 |
| 原始标题 | 单行文本 | 是 | 平台提取到的完整原始标题，不经 AI 改写 |
| 内容 ID | 单行文本 | 是 | 系统生成的全局唯一 ID；默认业务视图隐藏 |
| 原始链接 | URL | 是 | 用户发送的链接 |
| 规范链接 | URL | 是 | 解析跳转、移除追踪参数后的链接 |
| 平台 | 单选 | 是 | 微信公众号、掘金、小红书、抖音、B站、其他网页 |
...
| 阅读时间 | 日期时间 | 否 | 用户实际阅读内容的时间；新记录默认留空 |
...
> 请发送小红书、抖音、B站、微信公众号、掘金文章或其他公开网页链接。知归会自动提取内容、总结并保存到多维表格。
...
- 默认把规范化后的本地音频分段交给 faster-whisper，不发生第三方 ASR 数据传输；切换腾讯云时，其任务有效期、单请求大小限制和临时媒体清理策略必须在部署文档中明确。
- 腾讯模式会把待识别音频分段临时存入腾讯云 COS，并把短期预签名 URL 交给腾讯云 ASR 下载；Bucket、Region、对象前缀、URL 有效期和删除结果必须可审计但不得进入普通业务日志。
...
- PostgreSQL 由用户在 Compose 外自行部署和备份；任务状态通过 `KW_DATABASE_URL` 持久化到该数据库。
...
| AC-06 | 抖音或小红书视频在没有字幕时，默认通过本地 faster-whisper 完成转录，摘要依据转录内容生成。 |
...
| AC-20 | 配置可访问的用户自建 PostgreSQL 后，使用 Docker Compose 可以在干净环境中启动应用、通过健康检查，并在容器重启后恢复未完成任务。 |
| AC-21 | 首次绑定个人用户后，系统自动创建多维表格、内容库字段和默认视图并授予该用户可管理权限；重启或重复初始化不会产生第二套资源。 |
| AC-22 | 在本地 faster-whisper 与腾讯云 ASR 之间切换时，不修改业务用例、任务状态机、分段合并或归档代码。 |
| AC-23 | 腾讯云 ASR 路径能把本地音频上传到私有 COS、通过短期预签名 URL 完成识别，并按统一清理配置删除对象；清理失败产生可观察警告而不是静默残留。 |
...
- 默认模式完成 faster-whisper 模型缓存和真实音频契约测试。
- 若选择腾讯模式，确认 ASR 服务和 COS Bucket、Region、专用前缀可用，并验证上传、预签名读取和删除。
- 建立任务状态机、统一内容模型和错误码。
...
| B站提取 | 参考 [yt-dlp](https://github.com/yt-dlp/yt-dlp) 的公开接口与 DASH 处理策略，自建独立适配器 |
...
| D-07 | 原始媒体保存 | 默认不保留 | 原始音视频只作临时文件，任务结束后按配置删除 |
...
| D-11 | 默认 ASR | faster-whisper | 默认本地离线识别；保留腾讯云和其他 ASR 的统一端口与配置切换能力 |
| D-12 | 临时媒体存储 | 本地目录 | 路径和删除策略可配置；腾讯 ASR 固定在适配器内部使用私有 COS 短期中转 |
...
- [ ] 使用腾讯模式时，COS 临时对象上传、预签名读取和删除通过真实 Bucket 验证；共享 Bucket 不被系统擅自修改全局生命周期规则。
- [ ] 外部 API 限流和费用告警已配置。
